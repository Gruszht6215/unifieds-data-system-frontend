...
import { addTagToColumn, updateTagsColumnByColumnIdDispatch } from '../../redux/slices/importedDatabaseSlice'
...
export default function AddTagColumnFormModal({ isOpen, closeModal, column }: Props) {
    const dispatch = useAppDispatch()
    const { data, error, isLoading } = useSWR('getTagsByUserId', fetcher)
    const [currentTags, setCurrentTags] = useState(column.tags)
    const [availableTags, setAvailableTags] = useState<Tag[]>([])
    const [resetCurentTags, setResetCurrentTags] = useState(column.tags)
    ...
    const onClickConfirmHandler = () => {
        ...
        dispatch(updateTagsColumnByColumnIdDispatch(payloadBody))
        closeModal()
    }
    ...
    const onClickAddTagHandler = async () => {
        ...
        try {
            ...
            dispatch(addTagToColumn(payload))
            setNewTagName('')
